...
class ASGNN(nn.Module):
    """
    AS-GNN: structure-conditioned sparsification (no query-conditioning).
    Gate depends only on the triple's own structural context [h, r, t] --
    it does NOT see a query projection. This is ASR-GNN with the query_proj
    module and its gate input removed; everything else (data, training,
    eval) is byte-identical to train_asr_gnn.py so results are directly
    comparable.
    """
    ...
    def __init__(
        ...
        self.edge_gate = nn.Sequential(
            nn.Linear(emb_dim * 3, hidden_dim),   # [h, r, t] only -- was emb_dim*4 in ASR-GNN
            nn.ReLU(),
    ...
    def forward_components(self, triples: torch.Tensor):
        ...
        gate_input = torch.cat([h, r, t], dim=-1)   # no q
        alpha = self.edge_gate(gate_input)

        msg = self.msg_proj(torch.cat([h, r], dim=-1))
        sparse_msg = alpha * msg

        score = -torch.norm(h + sparse_msg - t, p=2, dim=-1)   # anchored on h, not q
...
def train(args: argparse.Namespace) -> None:
    ...
    save_json(os.path.join(args.output_dir, "status.json"), {"status": "started", "model": "AS-GNN", "dataset": args.dataset})
    ...
    print(f"Model: AS-GNN", flush=True)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train AS-GNN (structure-conditioned, no query) on static knowledge graphs.")
