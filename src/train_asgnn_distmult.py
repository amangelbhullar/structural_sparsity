...
class ASGNN(nn.Module):
    """
    AS-GNN (DistMult-like decoder): structure-conditioned sparsification,
    no query-conditioning. ASR-GNN-DistMult with query_proj and its gate
    input removed -- same edit pattern as the TransE-style AS-GNN.
    """
    ...
    def forward_components(self, triples: torch.Tensor):
        ...
        gate_input = torch.cat([h, r, t], dim=-1)
        ...
        emb_dim = h.size(-1)
        score = ((h + sparse_msg) * t).sum(dim=-1) / (emb_dim ** 0.5)
...
def train(args: argparse.Namespace) -> None:
    ...
    save_json(os.path.join(args.output_dir, "status.json"), {"status": "started", "model": "AS-GNN-DistMult", "dataset": args.dataset})
    ...
    print(f"Model: AS-GNN-DistMult", flush=True)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train AS-GNN-DistMult (structure-conditioned, no query) on static knowledge graphs.")
