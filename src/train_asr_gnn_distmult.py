...
class ASRGNN(nn.Module):
    """
    ASR-GNN: query-adaptive structural reasoning.
    Gate depends on the query context (h, r), unlike SD-GNN.
    """
    ...
    def forward_components(self, triples: torch.Tensor):
        ...
        q = self.query_proj(torch.cat([h, r], dim=-1))
        gate_input = torch.cat([h, r, t, q], dim=-1)
        ...
        emb_dim = q.size(-1)
        score = ((q + sparse_msg) * t).sum(dim=-1) / (emb_dim ** 0.5)
...
def train(args: argparse.Namespace) -> None:
    ...
    save_json(os.path.join(args.output_dir, "status.json"), {"status": "started", "model": "ASR-GNN", "dataset": args.dataset})
    ...
    print(f"Model: ASR-GNN", flush=True)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train ASR-GNN on static knowledge graphs.")
