#!/usr/bin/env python3
"""
RGAT + DistMult KGE experiment.

Architecture:
  - Encoder: Relational Graph Attention Network (RGAT)
    Uses RGATConv from PyG — attention-based soft sparsification.
    Unlike R-GCN (uniform aggregation), RGAT learns attention weights
    over neighbors — soft sparsity, not hard gating like SD/ASR-GNN.
  - Decoder: DistMult scoring

This fills the GAT row in the sparsification taxonomy table:
  Adaptive=Yes, Query-Aware=No, Sparsity=Soft, Efficiency=Low
  (dense computation remains despite attention weights)

Comparison role:
  DropEdge-RGCN  → random hard sparsity   MRR~0.093
  RGAT           → learned soft sparsity  MRR~?
  SD-GNN         → learned hard sparsity  MRR~0.267
  ASR-GNN        → query-adaptive hard    MRR~0.266
"""
...
class RGATEncoder(nn.Module):
    """
    Two-layer RGAT encoder using PyG's RGATConv.
    RGATConv computes attention weights per (edge, relation) pair —
    soft sparsification unlike RGCN's uniform aggregation.
    """

    def __init__(self, num_entities: int, num_relations: int,
                 hidden_dim: int = 200, dropout: float = 0.2,
                 num_heads: int = 4):
        super().__init__()
        self.entity_emb = nn.Embedding(num_entities, hidden_dim)
        self.dropout    = nn.Dropout(dropout)

        # RGATConv: multi-head attention over relational edges
        self.conv1 = RGATConv(
...
class RGATDistMult(nn.Module):
    def __init__(self, num_entities, num_relations, hidden_dim=200,
                 dropout=0.2, num_heads=4):
        super().__init__()
        self.encoder = RGATEncoder(num_entities, num_relations,
                                   hidden_dim, dropout, num_heads)
...
def train_epoch(model, optimizer, train_ids, edge_index, edge_type,
    ...
    for i in range(0, len(train_ids), batch_size):
        ...
        neg_h, neg_r, neg_t = negative_sample(bh, br, bt,
                                               model.num_entities, neg_per_pos)
        nh_emb = entity_repr[neg_h]
        nt_emb = entity_repr[neg_t]
...
def main():
    parser = argparse.ArgumentParser(description="RGAT + DistMult KGE")
    parser.add_argument("--dataset",     type=str, default="FB15k-237")
    parser.add_argument("--data-dir",    type=str, default="~/kg_experiments/data")
    parser.add_argument("--results-dir", type=str, default="~/kg_experiments/results/rgat")
    ...
    print(f"Test MRR={test_metrics['mrr']:.4f} H@10={test_metrics['hits_at_10']:.4f} "
          f"time={elapsed:.1f}s")
    ...
    outfile = results_dir / \
        f"{args.dataset}_RGAT-DistMult_ep{args.epochs}_seed{args.seed}.csv"
